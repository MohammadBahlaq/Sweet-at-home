...
class Data {
  ...
  static Icon loginOut = const Icon(Icons.login);
  ...
  static void getImages(int RID) {
    ...
    for (int i = 0; i < Results.length; i++) {
      if (Results[i]['RID'] == RID) {
        Images.add(Image.network("${Data.imgPath}${Results[i]['ImageName']}", fit: BoxFit.fill));
        ImagesSlider.add(Image.network("${Data.imgPath}${Results[i]['ImageName']}"));
  ...
  static void getFavoriteImages(int RID) {
    ...
    for (int i = 0; i < favorite.length; i++) {
      if (favorite[i]['RID'] == RID) {
        Images.add(Image.network("${Data.imgPath}${favorite[i]['ImageName']}", fit: BoxFit.fill));
        ImagesSlider.add(Image.network("${Data.imgPath}${favorite[i]['ImageName']}"));
  ...
  static getImagesUserRealEstate(int RID) {
    ...
    for (int i = 0; i < UserRealEstate.length; i++) {
      if (UserRealEstate[i]['RID'] == RID) {
        ...
        Images.add(Image.network("${Data.imgPath}${UserRealEstate[i]['ImageName']}", fit: BoxFit.fill));
        ImagesSlider.add(Image.network("${Data.imgPath}${UserRealEstate[i]['ImageName']}"));
  ...
  static getImagesOwnerRealEstate(int RID) {
    ...
    for (int i = 0; i < ownerRealEstate.length; i++) {
      if (ownerRealEstate[i]['RID'] == RID) {
        Images.add(Image.network("${Data.imgPath}${ownerRealEstate[i]['ImageName']}", fit: BoxFit.fill));
        ImagesSlider.add(Image.network("${Data.imgPath}${ownerRealEstate[i]['ImageName']}"));
